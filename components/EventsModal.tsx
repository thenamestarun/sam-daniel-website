  ...
  return (
    <>
      <button
        ...
        className="px-5 py-2.5 text-sm font-semibold tracking-wide backdrop-blur-md bg-[#f43f8a]/15 border border-[#f43f8a]/30 text-[#f43f8a] hover:bg-[#f43f8a]/25 hover:border-[#f43f8a]/50 transition-all rounded-sm"
      >
        Upcoming Events
      </button>
            ...
            <div className="max-h-[60vh] overflow-y-auto divide-y divide-white/6">
              {events.map((e) => (
                <div key={e._id} className="flex items-center justify-between px-6 py-4 hover:bg-white/3 transition-colors">
                  ...
                  <span className="text-white/35 text-xs font-light tracking-wide ml-4 flex-shrink-0">{e.dateLabel || (e.date ? formatEventDate(e.date) : "")}</span>
                </div>
              ))}
            </div>
